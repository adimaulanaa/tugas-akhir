...
class Internaldelevery_model extends CI_Model
{
    ...
    public function getNoFakturInternal($ymd)
    {
        $q = $this->db->query("SELECT MAX(RIGHT(no_faktur_internal,5)) AS id_max FROM tabel_internal WHERE substr(no_faktur_internal,6,6)='$ymd'");
        $kd = "";
        $kodeawal = "IN001";
        if ($q->num_rows() > 0) {
    ...
    public function getDataInternal($noresi, $username)
    {
        $this->db->where('tabel_internal.no_faktur_internal', $noresi);
        $this->db->where('tabel_internal.id_user', $username);
        $this->db->where('tabel_internal.selesai', '0');
        return $this->db->get('tabel_internal');
    }
    ...
    public function data_list_internal($nofak)
    {
        return $this->db->select('tabel_rinci_internal.*')
            ->where('no_faktur_internal', $nofak)
            ->get('tabel_rinci_internal')
            ->result();
    }
    ...
    public function getInternalSelesai($nofaktur, $id_user)
    {
        $this->db->where('no_faktur_internal', $nofaktur);
    ...
    {
        $this->db->where('no_faktur_internal', $nofaktur);
        return $this->db->get('tabel_rinci_internal');
    }
    ...
    public function get_list($no_faktur_internal)
	{
		return $this->db->select('tabel_rinci_internal.*')
			->where('tabel_rinci_internal.no_faktur_internal', $no_faktur_internal)
			->get('tabel_rinci_internal')
			->result();
    }
    ...
    public function detail_faktur($no_faktur_internal)
	{
		return $this->db->select('tabel_internal.*')
			->where('tabel_internal.no_faktur_internal', $no_faktur_internal)
			->where('tabel_internal.selesai', '1')
			->get('tabel_internal')
			->row();
    }
