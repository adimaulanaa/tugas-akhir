...
class Kasir extends CI_Controller
{
	...
	{
		$this->load->view('template/header');
		$this->load->view('kasir/update_faktur');
		$this->load->view('template/footer');
	}
	...
	{
		$this->load->view('template/header');
		$this->load->view('kasir/cetakfaktur');
		$this->load->view('template/footer');
	}

	public function get_data_faktur2()
	{
		...
		$data['produk'] = $this->kasir_model->getPenjualanSelesai($nofak, $id_user)->row();
		$data['produk'] = $this->kasir_model->getProdukDijual($nofak)->result();
		$hasil = $this->load->view('kasir/struk_transaksi', $data, true);
		$callback = array(
			'hasil' => $hasil,
		);
	...
	public function get_update_faktur()
	{
		$nofak = $this->input->post('nofak');
		$data['list'] = $this->kasir_model->get_list_faktur($nofak);
		$data['faktur'] = $this->kasir_model->detail_faktur($nofak);
		$hasil = $this->load->view('kasir/list_update_faktur', $data, true);
		$callback = array(
			'hasil' => $hasil,
		);
		echo json_encode($callback);
	...
	public function update_faktur_item()
	{
		$nofak = $this->uri->segment(3);
		// $kd_barang = base64_decode($this->uri->segment(4));
		$data_barang = $this->kasir_model->getProdukFaktur($nofak)->row();
		if ($data_barang) {
			$data['produk'] = $data_barang;
			$this->load->view('template/header', $data);
			$this->load->view('kasir/update_faktur_item');
			$this->load->view('template/footer');
		} else {
			$this->load->view('error404');
		}
	...
	public function simpan_update_faktur()
	{
		$nofak = $this->input->post('nofak');
		$total_penjualan = $this->input->post('total_penjualan');
		$diskon = $this->input->post('diskon');
		$total_penjualan_sdiskon = $this->input->post('total_penjualan_sdiskon');
		...
		$ket = $this->input->post('ket');
		$tgl = date('Y-m-d');
		$data_barang = $this->kasir_model->getProdukFaktur($nofak);
		$kd_toko = "SS001";
		$waktu = date('Y-m-d');
		$jam = date('H:i:s');
		$user = $this->session->userdata('ses_username');
		$this->db->query("INSERT INTO tabel_update_faktur (no_faktur_penjualan,tgl,total_penjualan,diskon,total_penjualan_sdiskon,ket_diskon,cash,debet,id_user,ket, bank) VALUES ('$nofak','$tgl','$total_penjualan','$diskon','$total_penjualan_sdiskon','$ket_diskon','$cash','$debet','$user','$ket','$bank')");
		$this->db->query("UPDATE tabel_penjualan SET tgl_penjualan='$tgl', total_penjualan='$total_penjualan', diskon='$diskon', total_penjualan_sdiskon='$total_penjualan_sdiskon', ket_diskon='$ket_diskon', cash='$cash', debet='$debet', id_user='$id_user', ket='$bank' WHERE no_faktur_penjualan='$nofak'");
		redirect('kasir/update_faktur/', 'refresh');
	}
	...
	public function mesin_kasir()
	{
		...
		if ($data_faktur) {
			...
			$data['member'] = $this->kasir_model->getMember();
			$this->load->view('template/header', $data);
	...
	public function go_to_bayar()
	{
		...
		$this->db->query("UPDATE tabel_rinci_penjualan SET kd_member='$kd_member', nm_member='$nm_member' WHERE no_faktur_penjualan='$noresi'");
	...
	// 			$subtot_sekarang = ($hrg_jual * $jum_beli_sekarang) - $diskonrp;
	...
	public function no_cetak_struk()
	{
		$tgl = date('Y-m-d');
		$waktu = date('H:i:s');
		$kd_toko = "SS001";
		...
		if ($data_faktur && $list_produk) {
			...
			$data_cetak['total_item'] = 0;
			$data_cetak['subtotal'] = 0;
			$this->load->view('kasir/no_struk_transaksi', $data_cetak);
			echo "<script>window.close();</script>";
		} else {
			echo "Error retrieving information from server. <br><br>Halaman ini tidak bisa dimuat ulang, silahkan tutup halaman ini.";
		}
