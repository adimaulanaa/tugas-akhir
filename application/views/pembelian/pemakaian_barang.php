<div class="content-wrapper" style="margin-bottom: 20px">
  <div class="container">
    <div class="row pad-botm">
      <div class="col-md-12">
        <h4 class="header-line">No. Faktur : <?php echo $faktur->no_faktur_pemakaiantoko ?></h4>
      </div>
    </div>
    <div class="row">
      <div class="col-md-12">
          ...
          <tbody>
            <tr>
              <!-- <div class="form-group">
                  <label for="gross_amount" class="col-sm-1 control-label" style="text-align:left;">Supplier1</label>
                  <div class="col-sm-3">
                    <select id="supplier" class="span6" required>
                      <option value="">Pilih Kategori</option>
                      <?php foreach ($supplier->result() as $key) : ?>
                        <option value="<?php echo $key->kd_supplier ?>"><?php echo $key->kd_supplier ?>|<?php echo $key->nm_supplier ?></option>
                      <?php endforeach ?>
                    </select>
                    <input type="text" class="form-control" id="supplier_1" onkeyup="copytextbox();" placeholder="Enter Customer Name" autocomplete="off" />
                  </div>
                </div> -->
              <!-- <div class="form-group">
                <label for="gross_amount" class="col-sm-1 control-label" style="text-align:left;">Supplier</label>
                <div class="col-sm-3">
                  <input type="text" class="form-control" id="date" placeholder="Enter Customer Name" autocomplete="off" />
                </div>
              </div> -->

              <input type="hidden" name="nofaktur" id="nofaktur" value="<?php echo $faktur->no_faktur_pemakaiantoko ?>">
              <!-- <input type="text" id="supplier_1" onkeyup="copytextbox();" required> -->
              ...
              <td><input type="text" style="width:25em" name="nm_barang" id="nm_barang" class="form-control" ></td>
        ...
        <form action="<?php echo base_url('pemakaiantoko/pemakaiantoko_selesai') ?>" id="formSelesai" class="form-inline" method="post">
          <input type="hidden" name="faktur_beli" id="faktur_beli" value="<?php echo $faktur->no_faktur_pemakaiantoko ?>">
          <input type="hidden" name="tot_harga" id="tot_harga">
          <!-- <input type="text" name="supplier_e" id="supplier_e"> -->
          <!-- <div class="form-group">
            <label for="gross_amount" class="col-sm-1 control-label" style="text-align:left;">Supplier</label>
            <div class="col-sm-5">
              <select class="form-control" name="supplier_e" id="supplier_e" required>
                <option value="">Pilih Supplier</option>
                <?php foreach ($supplier->result() as $key) : ?>
                  <option value="<?php echo $key->kd_supplier ?>"><?php echo $key->nm_supplier ?></option>
                <?php endforeach ?>
              </select>
            </div>
          </div> -->
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-horizontal" action="<?php echo base_url('pemakaiantoko/simpan_edit_item_beli') ?>" method="post">
          <div class="form-group">
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#idbarang').on('input', function() {
      ...
      $.ajax({
        type: "POST",
        url: "<?php echo base_url('pemakaiantoko/get_detail_produk') ?>",
        dataType: "JSON",
        data: {
          idbarang: idbarang
        },
    ...
    $('#btn_add_list').on('click', function(e) {
      ...
      var supplier_e = $('#supplier_e').val();
      var satuan = $('#satuan').val();
      $.ajax({
        type: "POST",
        url: "<?php echo base_url('pemakaiantoko/add_list_pemakaiantoko') ?>",
        dataType: "JSON",
        data: {
          idbarang: idbarang,
          nm_barang: nm_barang,
          nofaktur: nofaktur,
          supplier_e: supplier_e,
          jumlah: jumlah,
    ...
    function pemakaiantoko() {
      $.ajax({
        type: 'ajax',
        url: '<?php echo base_url() ?>pemakaiantoko/data_list_pemakaiantoko/<?php echo $this->uri->segment(3); ?>',
        async: true,
        dataType: 'json',
        success: function(data) {
          var html = '<p>Kosong</p>';
          ...
          for (i = 0; i < data.length; i++) {
            html += '<tr>' +
              ...
              '<td style="text-align:center;">' + '<a href="javascript:;" class="item_edit" faktur="' + data[i].no_faktur_pemakaiantoko + '" idbarang="' + data[i].kd_barang + '" jumlah="' + data[i].jumlah + '" harga="' + data[i].harga + '" nama="' + data[i].nm_barang + '">' + data[i].jumlah + ' ' + data[i].satuan + '</a>' + '</td>' +
    ...
    $('#show_data').on('click', '.item_hapus', function() {
      ...
      $.ajax({
        type: "POST",
        url: "<?php echo base_url('pemakaiantoko/hapus_item_beli') ?>",
        dataType: "JSON",
        data: {
          idbarang: idbarang,
          nofaktur: nofaktur,
    ...
    $('#btnSimpan').on('click', function() {
      ...
      $.ajax({
        type: "POST",
        url: "<?php echo base_url('pemakaiantoko/simpan_edit_jumlah_beli') ?>",
        dataType: "JSON",
        data: {
          nofaktur_e: nofaktur_e,
          idbarang_e: idbarang_e,
          jumlah_e: jumlah_e,
          harga_e: harga_e
        },
        success: function(data) {
