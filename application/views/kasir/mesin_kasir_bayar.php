<div class="content-wrapper" style="margin-bottom: 20px">
          ...
          <div class="panel-body">
            ...
            <div>
              <div class="form-inline">
                ...
                <br><br>
                <input style="text-align: right;" type="text" class="form-control pull-left" id="debet" placeholder="Debit (Rp)" name="debet">
                <!-- <input type="text" class="form-control pull-right" id="bank" placeholder="Bank" name="bank"> -->
                <select name="bank" id="bank" class="form-control pull-right">
            ...
            <hr>
            <div>
              <form target="printstruk1" id="formCetakStruk1" class="form-inline" action="<?php echo base_url('kasir/no-cetak-struk/') ?>" method="post">
                ...
                <button onclick="newTransaksi();" type="submit" id="btnCetakStruk1" class="btn btn-danger pull-left">NO PRINT</button>
              </form>
            </div>
            <div>
              <form target="printstruk" id="formCetakStruk" class="form-inline" action="<?php echo base_url('kasir/cetak-struk/') ?>" method="post">
                <input type="hidden" name="sum_print" id="sum_print" value="<?php echo $faktur->total_penjualan_sdiskon ?>">
                <input type="hidden" name="diskon_print" id="diskon_print" value="<?php echo $faktur->diskon ?>">
                <input type="hidden" name="nofak_print" id="nofak_print" value="<?php echo $faktur->no_faktur_penjualan ?>">
                <input type="hidden" name="cash_print" id="cash_print" value="0">
            ...
            <table id="tbUser" class="table table-bordered table-responsive">
              ...
              <tbody>
                ...
                <?php foreach ($list->result() as $key) : ?>
                  <tr>
                    <?php
                    $tot_satuan = $key->jumlah * $key->harga;
                    $sub_satuan += $tot_satuan;
                    $jml_diskon = $key->jumlah * $key->harga * $key->diskonpersen / 100;
                    $sum_jml_diskon +=  $jml_diskon;
                    $tot_item += $key->jumlah;
                    ...
                    <td align="right"><?php echo number_format($tot_satuan, 0, ',', '.') ?></td>
                    <td align="center"><?php echo $key->diskonpersen ?> %</td>
                    <td align="right"><?php echo number_format($jml_diskon, 0, ',', '.') ?></td>
              ...
              <tr>
                  ...
                  <td align="right"><strong >Rp. <?php echo number_format($sub_satuan, 0, ',', '.') ?></strong></td>
                  <td align="center"><strong><?php echo $tot_diskon ?> %</strong></td>
                  <td align="right"><strong>Rp. <?php echo number_format($sum_jml_diskon, 0, ',', '.') ?></td>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('#btnCetakStruk1').on('click', function(e) {
        var belanja = $('#sum_belanja').val();
        var cash = $('#cash_print').val();
        var debet = $('#debet_print').val();
        var kembali = (Number(cash) + Number(debet)) - Number(belanja);
        if (kembali < 0) {
          ...
          $('#formCetakStruk1').submit(function() {
            window.open('about:blank', 'printstruk1', 'width=500,height=400');
            this.target = 'printstruk1';
            window.location.href = "<?php echo base_url('kasir/transaksi-selesai/') ?>";
          });
        }
      });
