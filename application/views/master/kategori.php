  ...
  <div class="formSidebar w3-bar-block" style="right:0;padding:10px;" id="rightMenu">
    ...
    <form id="formTambah" action="<?php echo base_url('m_master/simpan_kategori') ?>" method="post">
      <table border="0" class="table" align="center">
        <tr>
          <td>Kode Kategori</td>
          <td>:</td>
          <td><input name="kd_kategori" type="text" id="kd_kategori" readonly /></td>
        </tr>
        <tr>
          <td>Nama Kategori</td>
          <td>:</td>
        ...
        <tbody>
          <?php foreach ($kategori->result() as $key) : ?>
            <tr>
              ...
              <td style="text-align: center;"><a href="javascript:void(0);" class="edit_record" data-kode="<?php echo $key->kd_kategori ?>" data-nama="<?php echo $key->nm_kategori ?>">Edit</a> | <a href="javascript:void(0);" class="delete-record" data-kd_kategori="<?php echo $key->kd_kategori ?>">Hapus</a></td>
            </tr>
          <?php endforeach ?>
        </tbody>
      </table>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form class="form-horizontal" action="<?php echo base_url('m_master/hapus_kategori') ?>" method="post">
            <h4>Apakah Kamu Yakin Menghapus Data Kategori Ini?</h4>
        </div>
        <input type="hidden" id="kd_kategori_h" name="kd_kategori_h">
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('#tbKategori').on('click', '.edit_record', function() {
        ...
        $('#formTambah').attr('action', '<?php echo base_url('m_master/simpan_kategori_edit') ?>');
      ...
      $('.delete-record').on('click', function() {
        var kode = $(this).data('kd_kategori');
        $('#modalHapus').modal('show');
        $('[name="kd_kategori_h"]').val(kode);
      });
    });
  </script>
